...
class MidtransService {
  ...
  Future midtransPaymentCheckout() async {
    ...
    String clientId = currentPaymentMethod.isTest == 1
        ? currentPaymentMethod.testValue!.midtransClientId.validate()
        : currentPaymentMethod.liveValue!.midtransClientId.validate();
    ...
    midpay.init(
      ...
      environment: currentPaymentMethod.isTest == 1
          ? Environment.sandbox
          : Environment.production,
    );

    var midtransCustomer = MidtransCustomer(appStore.userFirstName,
        appStore.userLastName, appStore.userEmail, appStore.userContactNumber);

    List<MidtransItem> listitems = [];

    var midtransTransaction = MidtransTransaction(
        totalAmount.toInt(), midtransCustomer, listitems,
        skipCustomer: true);
